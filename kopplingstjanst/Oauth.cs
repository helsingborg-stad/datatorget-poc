...
namespace kopplingstjanst
{
    static class Oauth
    {
        public static async Task<TokenResponse> RequestClientToken()
        {
            Thread.Sleep(2000);
            ...
            if (disco.IsError)
            {
                throw new Exception($"OAuth error: {disco.Error}");
            }
            ...
            {
                throw new Exception($"OAuth error: {tokenResponse.Error}");
            }
